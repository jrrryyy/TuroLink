...
import { Eye, EyeOff, Pencil, User as UserIcon, Bell as BellIcon } from 'lucide-react';
...
export default function AccountSettings() {
  ...
  const currentTab = searchParams.get('tab') === 'notifications' ? 'notifications' : 'profile';
  ...
  return (
    <DashboardLayout role={user.role} userName={user.name}>
      <div className="account-settings-wrap">
        <nav className="account-settings-nav-tabs" aria-label="Settings Navigation">
          <button
            type="button"
          ...
          <button
            type="button"
            className={`account-settings-tab-btn ${activeTab === 'notifications' ? 'active' : ''}`}
        ...
        {activeTab === 'notifications' ? (
          ...
          <form className="account-settings-card" ref={formRef} noValidate onSubmit={save}>
            ...
            {message && (
              <div className={success ? 'account-feedback success' : 'account-feedback'} role={success ? 'status' : 'alert'}>
                {message}
              </div>
            )}
            <fieldset disabled={busy}>
              <div className="account-profile-grid">
                <div className="account-profile-fields">
                  ...
                  <section className="account-details" aria-labelledby="account-details-heading">
                    <h2 id="account-details-heading">Account</h2>
                    <div className="account-details-table">
                      <div>Account Details</div>
                      <p>{user.email}</p>
                      <p>Password: <span aria-label="Password hidden">••••••••••••</span></p>
                    </div>
                  </section>
                </div>
                ...
                <div className="account-picture">
                  ...
                  <div className="account-picture-circle">
                    {image ? <img src={image} alt="Profile preview" /> : <span>{form.name?.trim().charAt(0).toUpperCase() || 'U'}</span>}
                  </div>
                  <label className="account-picture-edit">
                    <Pencil size={15} />
                    Edit
                    <input
                      ref={fileInput}
                  ...
                  {(photo || user.profilePicture || errors.profilePicture) && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (fileInput.current) fileInput.current.value = '';
                        setErrors((previous) => ({ ...previous, profilePicture: undefined }));
                      }}
                    >
                      Remove photo
                    </button>
              ...
              <section className="account-security" aria-label="Password and contact information">
                <div>
                  ...
                  <p className="account-hint">Leave these fields blank to keep your current password.</p>
                  {(user.hasPassword ? [['currentPassword', 'Current Password'], ['newPassword', 'New Password'], ['confirmPassword', 'Confirm New Password']] : []).map(([name, label]) => (
                    <label key={name}>
                      {label}
                      <div className="account-password">
                        ...
                        >
                          {visible[name] ? <EyeOff size={17} /> : <Eye size={17} />}
                        </button>
                      </div>
                      <FieldError errors={errors} name={name} />
                      {name === 'newPassword' && <PasswordStrength value={form.newPassword} />}
                    </label>
                  ))}
                  {!user.hasPassword && <p>You sign in with Google. Manage your password in your Google account.</p>}
                </div>
                <dl>
                  <dt>Contact Number</dt>
                  <dd>{user.phone || 'Not provided'}</dd>
                  <dt>Role</dt>
                  <dd>{user.role === 'teacher' ? 'Teacher' : 'Student'}</dd>
                </dl>
              </section>

              <div className="account-notification-link-box">
                <div className="account-notif-link-left">
                  <BellIcon size={22} className="account-notif-link-icon" />
                  <div>
                    <strong>Notification Preferences</strong>
                    <p>Customize email notifications, session reminders, message alerts, and push notifications.</p>
                  </div>
                </div>
                <button
                  type="button"
                  className="account-notif-link-btn"
                  onClick={() => selectTab('notifications')}
                >
